...
public class BoardDAO {
	...
	public int readList(int page) {
		...
		try {
			...
			String query = "select * from "
					...
					+ "(select * from MVC_BBS_BOARD order by bdgroup desc, bdorder)A ) "
					+ "where rnum >= ? and rnum <= ?";
	...
	private int[] paging(int page) throws Exception {
		...
		String query = "select count(*) from MVC_BBS_BOARD";
	...
	public int writeBoard(BoardDTO dto) {
		try {
			...
			if (dto.getBdGroup() < 0) {
				query = "insert into MVC_BBS_BOARD "
						+ "(bdnum, bdorder, bdindent, bdtitle, "
						+ "bdcontent, bduserid, bdviewcount,bdgroup, bddate) "
						+ "values (BDNUM_SEQ.nextval,?,?,?,?,?,?,BDGROUP_SEQ.nextval, sysdate)";
				preStmt = conn.prepareStatement(query);
				...
				query = "insert into MVC_BBS_BOARD "
						+ "(bdnum, bdorder, bdindent, bdtitle, "
						+ "bdcontent, bduserid, bdviewcount,bdgroup, bddate) "
						+ "values (BDNUM_SEQ.nextval,?,?,?,?,?,?,?, sysdate) ";
				preStmt = conn.prepareStatement(query);
				preStmt.setInt(7, dto.getBdGroup()); // 그룹넘버

				// 답글달기의 경우 먼저 같은 그룹의 나머지 글들의 순서를 1씩 증가시킴
				stmt = conn.createStatement();
				query = "update MVC_BBS_BOARD set bdorder = bdorder + 1 "
		...
		Exception e) {
			try {
				...
				System.out.println("BoardWriter rollback Ex");
	...
	public int readBoard(int bdNum) {
		...
		try {
			...
			String query = "select * from MVC_BBS_BOARD where bdnum = " + bdNum;
			result = stmt.executeQuery(query);
			int i = 0;
			while (result.next()) {
				i++;
			...
			if (session.getAttribute(view) == null) {
				query = "update MVC_BBS_BOARD set "
	...
	private void dbConnect() throws Exception {
		...
		datasource = (DataSource) context
				.lookup("java:comp/env/jdbc/Oracle12c");
	...
	public int Delete(int bdNum) {
		...
		try {
			dbConnect(); // db 연결
			stmt = conn.createStatement();
			String query = "delete from MVC_BBS_BOARD where bdnum = " + bdNum;
			result = stmt.executeQuery(query);
			...
			try {
				...
				rollbackEx.printStackTrace();
				System.out.println("Delete rollaback fail");
			}
			System.out.println("board db select fail");
			e.printStackTrace();
		} finally {
