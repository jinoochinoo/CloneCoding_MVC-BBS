...
public class UserInfoDAO {
	...
	public int registerUserInfo(UserInfoDTO dto) {
		...
		try {
			...
			if (check == Ctrl.TRUE) {
				...
				preStmt = conn.prepareStatement("insert into MVC_BBS_USER values (?,?,?,?,?,?)");
	...
	public int loginAccept() {
		...
		try {
			System.out.println("로그인 DAO 까지 옴");
			dbConnect();
			stmt = conn.createStatement();
			String userID = request.getParameter("userID");
			String userPW = request.getParameter("userPW");
			String query = "select userID, userPW from MVC_BBS_USER "
					+ "where userID = " + "'" + userID + "'";
	...
	public int getUserInfo() {
		...
		try {
			...
			dbConnect();
			stmt = conn.createStatement();
			String query = "select * from MVC_BBS_USER where userID = " + "'"
	...
	private int isUserID(UserInfoDTO dto) throws Exception {
		...
		// DB 연결
		dbConnect();
		String query = "select userID from MVC_BBS_USER " + "where userID = " + "'"
				+ dto.getUserID() + "'";
		stmt = conn.createStatement();
		result = stmt.executeQuery(query);
